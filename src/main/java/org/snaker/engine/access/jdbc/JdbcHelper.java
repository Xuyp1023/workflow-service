...
public abstract class JdbcHelper {
    ...
    private static void initialize() {
        final String driver = ConfigHelper.getProperty("jdbc.driver");
        final String url = ConfigHelper.getProperty("jdbc.url");
        final String username = ConfigHelper.getProperty("jdbc.username");
        final String password = ConfigHelper.getProperty("jdbc.password");
        final int maxActive = ConfigHelper.getNumerProperty("jdbc.max.active");
        final int maxIdle = ConfigHelper.getNumerProperty("jdbc.max.idle");
     ...
     * @throws SQLException
     */
    public static Connection getConnection(final DataSource ds) throws SQLException {
        // 通过ThreadLocale中获取Connection，如果为空，则通过dataSource返回新的连接对象
        final Connection conn = (Connection) TransactionObjectHolder.get();
        if (conn != null) {
            return conn;
        }
        if (ds != null) {
     ...
     * @return
     */
    public static <T> T requiredSingleResult(final Collection<T> results) {
        final int size = (results != null ? results.size() : 0);
        if (size == 0) {
            return null;
        }
     ...
     * @throws SQLException
     */
    public static Object getResultSetValue(final ResultSet rs, final int index, final Class<?> requiredType)
            throws SQLException {
     ...
     * @throws Exception
     */
    public static String getDatabaseType(final Connection conn) throws Exception {
     ...
     * @throws Exception
     */
    public static Dialect getDialect(final Connection conn) throws Exception {
        final DatabaseMetaData databaseMetaData = conn.getMetaData();
        final String databaseProductName = databaseMetaData.getDatabaseProductName();
        final String dbType = databaseTypeMappings.getProperty(databaseProductName);
     ...
     * @return 是否执行成功
     */
    public static boolean isExec(final Connection conn) {
        Statement stmt = null;
        try {
