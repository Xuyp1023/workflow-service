...
public class ProcessService extends AccessService implements IProcessService, CacheManagerAware {
	 ...
	 * @return
	 */
	public Process getProcessByName(String name, Long custNo) {
	 ...
	 * 先通过cache获取，如果返回空，就从数据库读取并put
	 */
	public Process getProcessByVersion(String name, Long custNo, Integer version) {
		...
		Process entity = null;
		String processName = name + DEFAULT_SEPARATOR + custNo + DEFAULT_SEPARATOR + version;
		Cache<String, Process> entityCache = ensureAvailableEntityCache();
		if(entityCache != null) {
			entity = entityCache.get(processName);
		}
		...
		List<Process> processs = access().getProcesss(null, new QueryFilter().setName(name).setCustNo(custNo).setVersion(version));
	 ...
	 * @param custNo 机构
	 */
	public String deploy(InputStream input, Long custNo) {
	...
	public String deploy(InputStream input, Long custNo, String creator) {
		AssertHelper.notNull(input);
		AssertHelper.notNull(custNo);
		try {
			byte[] bytes = StreamHelper.readBytes(input);
			ProcessModel model = ModelParser.parse(bytes);
			Integer version = access().getLatestProcessVersion(model.getName(), custNo);
	...
	private void cache(Process entity) {
		...
		String processName = entity.getName() + DEFAULT_SEPARATOR  + entity.getCustNo() + DEFAULT_SEPARATOR + entity.getVersion();
		if(nameCache != null && entityCache != null) {
			if(log.isDebugEnabled()) {
