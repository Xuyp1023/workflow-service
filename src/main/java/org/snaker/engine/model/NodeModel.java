...
public abstract class NodeModel extends BaseModel implements Action {
     ...
     * 前置局部拦截器实例集合
     */
    private final List<SnakerInterceptor> preInterceptorList = new ArrayList<SnakerInterceptor>();
    /**
     * 后置局部拦截器实例集合
     */
    private final List<SnakerInterceptor> postInterceptorList = new ArrayList<SnakerInterceptor>();
     ...
     * @param execution 执行对象
     */
    protected void runOutTransition(final Execution execution) {
        for (final TransitionModel tm : getOutputs()) {
            tm.setEnabled(true);
